...
final class GregorianFormat
{
    ...
    public String format(final int astronomicalYear, final int month, final int dayOfMonth, final int hour, final int minute, final int integralSec, final BigDecimal fractional, final int timezone)
    {
        ...
        {
            // [-]YYYY
            appendDigits(astronomicalYear, MIN_YEAR_DIGITS, buffer);

            if (m_hasMonth)
            {
                // [-]YYYY-MM
                buffer.append(DATE_SEPARATOR);
                appendDigits(month, MONTH_DIGITS, buffer);

                if (m_hasDayOfMonth)
                {
                    // [-]YYYY-MM-DD
                    buffer.append(DATE_SEPARATOR);
                    appendDigits(dayOfMonth, DAYOFMONTH_DIGITS, buffer);

                    if (m_hasTime)
                    {
                        // [-]YYYY-MM-DDTHH:MM:SS.SSS
                        buffer.append(DATETIME_SEPARATOR);
                        appendTimePart(hour, minute, integralSec, fractional, buffer);
                    }
                    else
                    {
                        // [-]YYYY-MM-DD
