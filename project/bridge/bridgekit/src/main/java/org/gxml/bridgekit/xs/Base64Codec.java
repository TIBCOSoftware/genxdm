...
/**
 * Encoding and decoding using the base 64 algorithm.
 * <p/>
 * This class provides a number of static utility functions useful in encoding and decoding materials received in a MIME message. This codec handles Base64.
 * 
 * @author Amelia A Lewis &lt;alewis@apache.com>
 * @version $Id: Base64Codec.java,v 1.10 2005/12/28 20:34:26 dholmes Exp $
 */
...
final class Base64Codec
{
	...
	/**
	 * Warning: if the supplied string does not work out to a multiple of 3 bytes, then the returned string will contain padding. If it has padding, then it cannot have another string concatenated on the end of it.
	 * 
	 * @param raw
	 *            the String to be encoded
	 * @return a String with length a multiple of four, which contains the base64 encoded version of the input with line breaks when needed and a closing line-break as well. Line breaks are CRLF as per the MIME RFC.
	 ...
	 *            if true a single line without line-ending is returned, otherwise lines are folded after the 76th character and a closing line break (CRLF) is appended at the end as well.
	...
	/**
	 * Warning: if the supplied byte array does not have length a multiple of 3 bytes, then the returned string will contain padding. If it has padding, then it cannot have another string concatenated on the end of it.
	 * 
	 * @param raw
	 *            the byte array
	 * @param dontBreakLines
	...
	/**
	 * Implementation detail: package access. AS has shown by profiling that these two methods are faster than table lookup. Note that the Base64 Table remains defined in the constants, as it is useful as documentation (if nothing else).
	 * 
	 * @param sixBit
	 ...
	 * @return a value in the range -1..63, which is either six bits of a larger bit pattern (0-63), or an error indicator (-1).
	 */
